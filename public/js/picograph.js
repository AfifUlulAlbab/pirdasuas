...
function createLegendRect(labelDivID, color, label, valueID) {
    ...
    const valueSpan =
        label.at(-1) == ":" ? `<span id="${valueID}"></span>` : "";
...
class Graph {
    ...
    updateMinMax(value) {
        ...
        let newMaxVal =
            Math.ceil(
                Math.max.apply(Math, this.points.flat().filter(Number.isFinite))
            ) +
            valueMinMaxDelta * 0.05;

        let newMinVal =
            Math.floor(
                Math.min.apply(Math, this.points.flat().filter(Number.isFinite))
            ) -
            valueMinMaxDelta * 0.05;
        ...
        if (this.autoScaleMode == 2) {
            ...
            /* autoScaleMode == 1 */
            if (value > this.maxVal - valueMinMaxDelta * 0.05)
                this.maxVal = newMaxVal;

            if (value < this.minVal + valueMinMaxDelta * 0.05)
                this.minVal = newMinVal;
        }
    }
    ...
    updateLegends(values) {
        for (let i = 0; i < this.noLabels; i++)
            byID(this.valueIDs[i]).innerHTML =
                values[i].toFixed(2) + " " + this.unit;
    }

    updateTimestamps() {
        const timestampString = getTimestamp();

        this.timestampsArray = shiftArrayLeft(
    ...
    drawHorizontalLines() {
        ...
        for (let i = 1; i <= this.scalesteps; i++) {
            ...
            this.ctx.fillText(
                (i * this.sstep + this.minVal).toFixed(2) +
                    " " +
                    entityDecode.value,
                xoffset,
                y + yoffset
    ...
    drawTimestamps() {
        const xBoundPix = this.ctx.measureText(
            (this.scalesteps * this.sstep).toFixed(2)
        ).width;
        ...
        for (let i = this.nPoints - 1; i >= xBound; i -= this.vlinesFreq) {
            ...
            const yoffset =
                this.ctx.measureText(this.timestampsArray[i]).width +
                4 * this.cssScale;
            this.ctx.rotate(Math.PI / 2);
            this.ctx.fillText(
                this.timestampsArray[i],
                this.height - yoffset,
                -x + xoffset
            );
            this.ctx.stroke();
...
function emptyArray2D(nrows, ncols, fill = undefined) {
    return Array.from({ length: nrows }, (_) =>
        Array.from({ length: ncols }, (_) => fill)
    );
}
